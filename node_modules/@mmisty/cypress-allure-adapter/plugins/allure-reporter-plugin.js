...
const createNewContentForContainer = (nameAttAhc, existingContents, ext, specname) => {
    const getContentJson = () => {
        try {
            ...
            console.error(`${common_1.packageLog} Could not parse the contents of attachment ${nameAttAhc}`);
            return {};
        }
    };
    ...
    screenshotOne(arg) {
        ...
        files.forEach(file => {
            ...
            if (!(0, fs_1.existsSync)(file)) {
                console.log(`${common_1.packageLog} file ${file} doesnt exist`);
                return;
            }
    ...
    attachVideoToContainers(arg) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            (0, fs_1.readFile)(videoPath, errVideo => {
                if (errVideo) {
                    console.error(`${common_1.packageLog} Could not read video: ${errVideo}`);
                    return;
                }
                testsAttach.forEach(test => {
                    if (!test.parent) {
                        console.error(`${common_1.packageLog} not writing videos since test has no parent suite: ${test.fullName}`);
                        return;
                    }
